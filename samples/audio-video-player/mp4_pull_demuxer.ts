import { AUDIO_STREAM_TYPE, PullDemuxerBase } from '../lib/pull_demuxer_base.js'
// import  '../third_party/mp4boxjs/mp4box.all.min.js'
...
export class MP4PullDemuxer extends PullDemuxerBase {
  ...
  async initialize(streamType: any) {
  ...
  audioTrack(audioTrack: any) {
    throw new Error('Method not implemented.')
  }
  videoTrack(videoTrack: any) {
    throw new Error('Method not implemented.')
  }
  ...
  _onSamples(samples: string | any[]) {
    const SAMPLE_BUFFER_TARGET_SIZE = 50

    this.readySamples.push(...samples)
    if (this.readySamples.length >= SAMPLE_BUFFER_TARGET_SIZE) this.source.stop()
