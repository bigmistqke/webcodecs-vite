...
let dec_time = {
  all: [] as Array<readonly [number, number]>,
  min: Number.MAX_VALUE,
  max: 0,
  sum: 0,
}

const encqueue_aggregate = {
  all: [] as Array<number>,
  min: Number.MAX_VALUE,
  max: 0,
  avg: 0,
...
function enc_update(data: { output: number; timestamp: number; time: number }) {
  enc_aggregate.all.push(data)
  //self.postMessage({text: 'enc_update called: ' + JSON.stringify(data)});
}

function encqueue_update(duration: number) {
...
function enc_report() {
  ...
  for (let i = 1; i < len; i++) {
    if (
      enc_aggregate.all[i].output == 1 &&
      enc_aggregate.all[i - 1].output == 0 &&
      enc_aggregate.all[i].timestamp == enc_aggregate.all[i - 1].timestamp
    ) {
...
function encqueue_report() {
  ...
  const fquart =
    encqueue_aggregate.all[f] + alpha1 * (encqueue_aggregate.all[f + 1] - encqueue_aggregate.all[f])
  const tquart =
    encqueue_aggregate.all[t] + alpha3 * (encqueue_aggregate.all[t + 1] - encqueue_aggregate.all[t])
  const median =
    len % 2 === 1
...
function dec_update(data: { output: number; timestamp: any; time: number }) {
...
function dec_report() {
  ...
  for (let i = 1; i < len; i++) {
    if (
      dec_aggregate.all[i].output == 1 &&
      dec_aggregate.all[i - 1].output == 0 &&
      dec_aggregate.all[i].timestamp == dec_aggregate.all[i - 1].timestamp
    ) {
      const timestamp = dec_aggregate.all[i].timestamp
      const dec_delay = dec_aggregate.all[i].time - dec_aggregate.all[i - 1].time
      const data = [timestamp, dec_delay] as const
...
function decqueue_update(duration: number) {
...
function decqueue_report() {
  ...
  const alpha3 = (3 * (len + 1)) / 4 - Math.trunc((3 * (len + 1)) / 4)
  const fquart =
    decqueue_aggregate.all[f] + alpha1 * (decqueue_aggregate.all[f + 1] - decqueue_aggregate.all[f])
  const tquart =
    decqueue_aggregate.all[t] + alpha3 * (decqueue_aggregate.all[t + 1] - decqueue_aggregate.all[t])
  const median =
    len % 2 === 1
...
interface PipelineConfig extends VideoEncoderConfig {
  decHwAcceleration?: HardwareAcceleration | undefined
}
...
class Pipeline {
  ...
  constructor({
    ...
    streams: { input: ReadableStream; output: WritableStream }
    config: PipelineConfig
  }) {
    this.stopped = false
    this.inputStream = streams.input
    this.outputStream = streams.output
    this.config = config
  }

  DecodeVideoStream(self: Window & typeof globalThis) {
          ...
          if (chunk.type == 'config') {
            ...
            try {
              ...
              if (decoderSupport.supported) {
                ...
                self.postMessage({
                  severity: 'fatal',
                  text: 'Decoder Config not supported:\n' + JSON.stringify(decoderSupport.config),
                })
              }
            } catch (e) {
              self.postMessage({
                severity: 'fatal',
                text: `Decoder Configuration error: ${e.message}`,
              })
            }
          } else {
            try {
  ...
  EncodeVideoStream(self: Window & typeof globalThis, config: PipelineConfig) {
    let frameCounter = 0
    let seqNo = 0
    let keyframeIndex = 0
    let deltaframeIndex = 0
    ...
    return new TransformStream({
          ...
          output: (chunk, cfg) => {
            if (cfg?.decoderConfig) {
              ...
              const configChunk = {
                ...
                deltaframeIndex: deltaframeIndex,
                timestamp: 0,
                pt: 0,
            ...
            pending_outputs--
            chunk.seqNo = seqNo
            chunk.keyframeIndex = keyframeIndex
            chunk.deltaframeIndex = deltaframeIndex
      ...
      transform: (frame, controller) => {
        if (pending_outputs <= 30) {
          pending_outputs++
          const insert_keyframe = frameCounter % config.keyInterval == 0
          frameCounter++
          try {
  ...
  stop() {
    ...
    if (len > 1) {
      ...
      self.postMessage({
        severity: 'chart',
        x: 'Frame Number',
        y: 'Glass-Glass Latency',
        label: 'Glass-Glass Latency (ms) by Frame Number',
        div: 'chart2_div',
        text: '',
      })
      self.postMessage({
        ...
        x: 'Timestamp',
        y: 'Encoding Time',
        label: 'Encoding Time (ms) by Timestamp',
        div: 'chart3_div',
        text: JSON.stringify(enc_time.all),
      })
      self.postMessage({
        severity: 'chart',
        x: 'Timestamp',
        y: 'Decoding Time',
        label: 'Decoding Time (ms) by Timestamp',
        div: 'chart4_div',
        text: JSON.stringify(dec_time.all),
      })
